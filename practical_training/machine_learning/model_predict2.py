...
def get_model():
	...
	model.load("./tflearn_data/tflearn_models/2019-07-05 00.52.07.451486(10, 42)/model")
	return model


def comment_analysis(model):
	...
	X_train_word_ids = convert_X_to_X_word_ids(X)

	# 序列扩充
	X_train_padded_seqs = pad_sequences(X_train_word_ids, maxlen=20, value=0)

	Y = model.predict(X_train_padded_seqs)
	print(Y)


if __name__ == '__main__':
	model = get_model()
	comment_analysis(model)
